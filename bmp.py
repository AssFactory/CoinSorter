def bmp(fname, display, x, y, color=0):
	...
	if b[0]==0x42 and b[1]==0x4D:
		...
		d_buffer = bytearray(height*width*2)
		...
		for row in range(height):
			...
			for index in range(width):
				...
				if color:
					...
					d_buffer[(index*2)+((height-row-1)*width*2)] = c >> 8
					d_buffer[(index*2+1)+((height-row-1)*width*2)] = c & 0xff			
				else:
					if buffer[index*3]!=0xff:
						display.pixel(x,y,1)
			
		display.draw_bmp(x,y,width,height,d_buffer)
	f.close()
